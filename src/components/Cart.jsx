...
function Cart({ setShowCart }) {
  ...
  return (
    <div className="fixed w-full sm:w-96 p-2 bg-slate-100 h-screen top-0 right-0 z-50 rounded-l-md border-l-2">
          ...
          <div className="p-2 h-[90%] flex flex-col gap-2 justify-between">
            <div className="flex flex-col overflow-auto no-scrollbar">
              {cart?.map((c) => {
                return (
                  <div
                    className="p-1 px-2 h-16 flex gap-2 items-center bg-white border-2 rounded-md"
                    key={c.id}
                  >
                    <img
                      src={c.imgCapture}
